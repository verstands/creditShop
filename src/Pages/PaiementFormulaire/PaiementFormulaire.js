...
const PaiementFormulaire = () => {
    ...
    const PayBtn = async (e) => {
        ...
        setloadingA(true)
        try {
            const data = `USD/${numero}/${dataa}/creditshop`
            await addPayer(data)
            setloadingA(false)
        } catch (error) {
            setloadingA(false)
                    ...
                    <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <form ref={form} onSubmit={PayBtn} class="space-y-4 md:space-y-6">
                            <div>
                                ...
                                <input value={numero} onChange={handleNumeroChange} type="number" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="" required="" />
                            </div>
